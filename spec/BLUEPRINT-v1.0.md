...
**Ledger entry bytes.** The same exclusion rule defines the bytes a ledger entry's digest and signature are computed over. An entry's digest is computed over the entry's exact stored UTF-8 bytes with the values of the entry's own digest field and its own signature field replaced by the empty string, both fields remaining present; the entry's signature is computed over the same excluded byte sequence, so a signature covers the previous-digest link and every claim in the entry but never itself or the digest it accompanies. Where a family's schema names these fields differently, the excluded fields are the entry's own digest and its own signature, whatever their names. Verification recomputes under the same exclusions; no other canonicalization is applied. [spec_basis: proposed, same basis and docket routing as the clause above.]
...
**Chain head sentinel.** The first entry of a hash chained family carries, in its previous-digest field, the genesis sentinel: `sha256:` followed by sixty-four zero characters. The sentinel satisfies the digest pattern every entry schema requires, names no real entry, and makes the chain head detectable rather than conventional: a verifier walks back to the sentinel or the chain is broken. A family MUST NOT contain the sentinel in any position but its first entry. [spec_basis: proposed. No clause defined the chain head value and the entry schemas make the field mandatory, so two implementations would have diverged silently; enters the section 14 Draft docket loop.]
