...
The session entry contract is what a session close hook can honestly supply. A session granularity entry MUST carry the session identifier, the harness identifier from the TRACE/1.0 harness registry where one exists, the provider identifier of the provider record it cites, the session start and end times, the count of calls the entry covers and the counts refused and failed among them, the weakest custody class observed across the covered calls, and the evidence grade of each claim, which for a hook written entry is attested. A session that used more than one provider writes one session entry per provider, so that every entry cites exactly one provider record. Per call fields that the hook cannot honestly supply, such as per call input record citations, payload hashes, and per call verdicts, are absent at Tier 1. An absent field MUST be absent rather than fabricated: a session entry MUST NOT carry a synthesized per call value, a placeholder hash, or a count the hook did not observe, because a ledger entry's worth is exactly the honesty of its claims. A session granularity entry conforms to `schema/v1/session-inference.schema.json`; the per call form of `schema/v1/inference-call.schema.json`, whose required fields a session close hook cannot honestly supply, binds where per call entries are written, which section 12.1's table places at Tier 2.
